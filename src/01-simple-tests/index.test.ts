...
describe('simpleCalculator tests', () => {
  ...
  test('should return null for invalid arguments', () => {
    const result = simpleCalculator({ a: 'пять', b: 3, action: Action.Add });
    expect(result).toBeNull();
  });

  test('should return null for partially invalid input', () => {
    const result = simpleCalculator({ a: 5, b: 'three', action: Action.Add });
    expect(result).toBeNull();
  });
